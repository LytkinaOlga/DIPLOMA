...
export default function FlowRenderer({ initFlowId, initFlowName, flowNodes, flowEdges, initFlowDescription }) {
    ...
    const onNodeClick = (event, node) => {
        ...
        console.log(task);
        const selectedTaskParams = JSON.parse(JSON.stringify(task[0].parameters));
    ...
    const fillNodeName = (nodeName, nodeId) => {
        console.log(nodes);
        const node = nodes.filter(node => node.id === nodeId);
